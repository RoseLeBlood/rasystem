...
{
   fclose(image->file);
   SAFE_MEMFREE(image->tmp);
   SAFE_MEMFREE(image->tmpR);
   SAFE_MEMFREE(image->tmpG);
   SAFE_MEMFREE(image->tmpB);
   SAFE_MEMFREE(image->rowSize);
   SAFE_MEMFREE(image->rowStart);
   SAFE_MEMFREE(image);
}
...
unsigned  char* System::raISGI::LoadSGI(raString name)
{
	...
	image = ImageOpen(name.c_str());

	if(!image)
		return NULL;

	m_bmpi->bmiHeader.biWidth = image->xsize;
	m_bmpi->bmiHeader.biHeight = image->ysize;
	m_zSize = image->zsize;
