...
#define ERROR_RESOURCE_VALUE 1
	...
	typedef void ( CALLBACK*LPCREATETEXTUREFROMFILE11 )( raDirectX* pDev,  raString szFileName,
														 ID3D11ShaderResourceView** ppRV, void* pContext );
	typedef void ( CALLBACK*LPCREATEVERTEXBUFFER11 )( raDirectX* pDev, ID3D11Buffer** ppBuffer,
													  D3D11_BUFFER_DESC BufferDesc, void* pData, void* pContext );
	typedef void ( CALLBACK*LPCREATEINDEXBUFFER11 )( raDirectX* pDev, ID3D11Buffer** ppBuffer,
													 D3D11_BUFFER_DESC BufferDesc, void* pData, void* pContext );
	...
	protected:
		void                            LoadMaterials( raDirectX* pd3dDevice, SDKMESH_MATERIAL* pMaterials,
													   UINT NumMaterials, SDKMESH_CALLBACKS11* pLoaderCallbacks=NULL );

		HRESULT                         CreateVertexBuffer( raDirectX* pd3dDevice,
															SDKMESH_VERTEX_BUFFER_HEADER* pHeader, void* pVertices,
															SDKMESH_CALLBACKS11* pLoaderCallbacks=NULL );

		HRESULT                         CreateIndexBuffer( raDirectX* pd3dDevice, SDKMESH_INDEX_BUFFER_HEADER* pHeader,
														   void* pIndices, SDKMESH_CALLBACKS11* pLoaderCallbacks=NULL );

		virtual HRESULT                 CreateFromFile( raDirectX* pDev11,
														LPCTSTR szFileName,
														bool bCreateAdjacencyIndices,
														SDKMESH_CALLBACKS11* pLoaderCallbacks11 = NULL );

		virtual HRESULT                 CreateFromMemory( raDirectX* pDev11,
														  BYTE* pData,
														  UINT DataBytes,
														  bool bCreateAdjacencyIndices,
														  bool bCopyStatic,
	...
	public:
		...
		virtual HRESULT                 Create( raDirectX* pDev11, LPCTSTR szFileName, bool bCreateAdjacencyIndices=
												false, SDKMESH_CALLBACKS11* pLoaderCallbacks=NULL );
		virtual HRESULT                 Create( raDirectX* pDev11, BYTE* pData, UINT DataBytes,
		...
		virtual void                    Render( UINT iDiffuseSlot = INVALID_SAMPLER_SLOT,
												UINT iNormalSlot = INVALID_SAMPLER_SLOT,
												UINT iSpecularSlot = INVALID_SAMPLER_SLOT );
		virtual void                    RenderAdjacent( UINT iDiffuseSlot = INVALID_SAMPLER_SLOT,
														UINT iNormalSlot = INVALID_SAMPLER_SLOT,
														UINT iSpecularSlot = INVALID_SAMPLER_SLOT );
